...
class Menu_ImageSizeSettings: UITableViewController
{
    public weak var Delegate: Menu_ImageSizeProtocol? = nil
    
    override func viewDidLoad()
    {
        super.viewDidLoad()
        ShowDescription()
        OriginalSize = Settings.GetInteger(ForKey: .MaxImageDimension)
        if OriginalSize == 0
        {
            OriginalSize = 512
            Settings.SetInteger(OriginalSize, ForKey: .MaxImageDimension)
        }
        if let Index = SizeMap[OriginalSize]
        {
    ...
    {
        if OriginalSize != Settings.GetInteger(ForKey: .MaxImageDimension)
        {
            Menu_ChangeManager.AddChanged(.MaxImageDimension)
            Delegate?.ChangesFromChild([.MaxImageDimension])
        }
    }
